...
abstract class PlaylistDao : BaseDao<DbPlaylist>() {
    ...
    abstract fun orderByName(): DataSource.Factory<Int, DbPlaylist>

    @Query("SELECT * FROM playlist WHERE playlist.name LIKE :filter ORDER BY name COLLATE UNICODE")
    abstract fun orderByNameFiltered(filter: String): DataSource.Factory<Int, DbPlaylist>

    @Query("SELECT * FROM playlist WHERE playlist.name LIKE :filter ORDER BY name COLLATE UNICODE")
    abstract suspend fun orderByNameFilteredList(filter: String): List<DbPlaylist>
}