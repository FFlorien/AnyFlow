...
abstract class LibraryDatabase : RoomDatabase() {
    ...
    fun setCurrentFilters(filters: List<Filter<*>>): Completable =
            asyncCompletable(CHANGE_FILTER_GROUP) {
                getFilterGroupDao().deleteGroup(1)
    ...
    fun getAlbumArtsForFilterGroup(): Flowable<List<List<String>>> = getFilterGroupDao()
            .allSavedFilterGroup()
            .map { groups -> groups.map { group -> getFilterDao().filterForGroupSync(group.id) } }
    ...
    fun deleteFilterGroups(groupsToDelete: List<FilterGroup>): Completable {
        return if (groupsToDelete.isEmpty()) {
            Completable.complete()
        } else {
            Completable.fromAction {
                groupsToDelete.forEach {
                    getFilterGroupDao().deleteGroup(it.id)
                }
            }
        }.subscribeOn(Schedulers.io())
    }

    fun updateFilterGroup(newFilterGroup: FilterGroup): Completable = Completable.fromAction {
